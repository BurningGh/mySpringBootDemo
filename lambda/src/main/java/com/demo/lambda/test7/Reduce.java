...
public class Reduce {
    public static void main(String[] arg) {
        ...
        List<String> colors = apples.stream().map(Apple::getColor).collect(Collectors.toList());//从apple中提取颜色
        ...
        Optional<Apple> first = apples.stream().filter(apple -> apple.getWeight() > 1).findFirst();
        System.out.println(first.isPresent() ? first.get() : "");

        //count统计流中的元素个数
        System.out.println("count:" + (long) apples.size());
        ...
        //Integer中提供了静态的sum和max方法，代码可以进一步简化
        ...
        */
        ...
        System.out.println("weight:" + apples.stream().filter(apple -> apple.getWeight() > 1).filter(apple -> apple.getColor().equals("red")).map(Apple::getWeight).reduce(0, Integer::sum));
    }
}
